...
if __name__ == '__main__':
    ...
    for i, type in enumerate(types):
        ...
        bic[i] = gmm.bic(x)
    print('错误率：', err.ravel())
    print('BIC：', bic.ravel())  # bic——当前模型对输入x的贝叶斯信息准则  值越小越好
    xpos = np.arange(4)
    plt.figure(facecolor='w')
    ax = plt.axes()
    b1 = ax.bar(xpos-0.3, err, width=0.3, color='#77E0A0', edgecolor='k')
    b2 = ax.twinx().bar(xpos, bic, width=0.3, color='#FF8080', edgecolor='k')  # twinx 表示添加一个y轴
    ...
    optimal = bic.argmin()
    gmm = GaussianMixture(n_components=2, covariance_type=types[optimal], random_state=0)  # 选择bic最小的
    gmm.fit(x)
    print('均值 = \n', gmm.means_)
    print('方差 = \n', gmm.covariances_)
    y_hat = gmm.predict(x)
    ...
    if gmm.means_[0][0] < gmm.means_[1][0]:  # 为了画图的需要，将结果进行取反
        z = grid_hat == 0
        grid_hat[z] = 1
        grid_hat[~z] = 0
    plt.figure(facecolor='w')
    ...
    CS = plt.contour(x1, x2, p, levels=(0.1, 0.5, 0.8), colors=list('rgb'), linewidths=2)  # 绘制等高线contour(X,Y,Z)
