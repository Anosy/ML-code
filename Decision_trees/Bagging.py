...
if __name__ == '__main__':
    ...
    x = np.sort(x)
    y = f(x) + 0.05 * np.random.randn(N) # 添加噪声
    x = x.reshape(-1, 1)
    ...
    bagging_dtr = BaggingRegressor(dtr, n_estimators=100, max_samples=0.2)
    regs = [
        ('DecisionTree Regression', dtr),
        ('Ridge Regressor(%d Degree)' % degree, ridged),
        ('Bagging Ridge(%d Degree)' % degree, bagging_ridged),
        ('Bagging DecisionTree Regression', bagging_dtr)
    ]
