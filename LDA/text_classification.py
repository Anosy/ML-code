...
def test_clf(clf):
    ...
    model = GridSearchCV(clf, param_grid={'alpha': alpha_can}, cv=3)
    m = alpha_can.size
    print('m: ', m)
    if hasattr(clf, 'alpha'):  # 判断返回的对象是否含有给定名称的属性
    ...
    if hasattr(clf, 'n_clusters'):
        n_clusters_can = np.arange(4,7)
        model.set_params(param_grid={'n_clusters': n_clusters_can})
    if hasattr(clf, 'n_components'):
        n_components_can = np.arange(4, 7)
        model.set_params(param_grid={'n_components': n_components_can})
    ...
    t_end = time()
    t_train = (t_end - t_start) / (3 * m)
    print('3折交叉验证的训练时间为：%.3f秒/(5*%d)=%.3f秒' % ((t_end - t_start), m, t_train))
...
if __name__ == '__main__':
    ...
    data_train = fetch_20newsgroups(data_home=corpus_path, subset='train', categories=categories, shuffle=True,
                                    random_state=0, remove=remove)
    data_test = fetch_20newsgroups(data_home=corpus_path, subset='test', categories=categories, shuffle=True,
                                   random_state=0, remove=remove)
    t_end = time()
    ...
    print('\n\n===================\n分类器的比较：\n')
    clfs = (
            KMeans(n_clusters=4, init='k-means++', n_init=5),
            MultinomialNB(),  # 0.87(0.017), 0.002, 90.39%
    ...
    plt.xticks(x+0.5, names)  # 给x轴添加上信息
